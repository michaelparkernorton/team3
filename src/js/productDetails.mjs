...
export default async function productDetails(productId) {
  ...
  try {
    renderProductDetails(); // This function does not account for invalid products name.
  } catch (err) {
    // the error is been handled wih this try catch block
    if (err) {
      document.querySelector(".product-detail").innerHTML =
        "The product is not currently available"; // this message is displaped to the user if error is found
    }
  }
